...
void  v2ProfileEp()
{
    ...
    TFile *file = new TFile("Data/Results/Run11_200GeV/testing/Run11_200GeV_femtoTest02.root");
    ...
    TCanvas *cv22posTests[numCentBins];
    TCanvas *cv22negTests[numCentBins];
    
    char testName[256];
    char testTitle[256];
    for(int i = 0; i < numCentBins; i++)
    {
        sprintf(testName,"cv22p%i",i);
        sprintf(testTitle,"Plots/testPlots/v22EPplu%i.pdf",i);
        cv22posTests[i] = new TCanvas(testName);
        v2_2Plus[i]->Draw();
        tline->Draw("same");
        cv22posTests[i]->SaveAs(testTitle);

        sprintf(testName,"cv22m%i",i);
        sprintf(testTitle,"Plots/testPlots/v22EPmin%i.pdf",i);
        cv22negTests[i] = new TCanvas(testName);
        v2_2Minus[i]->Draw();
        tline->Draw("same");
        cv22negTests[i]->SaveAs(testTitle);
    ...
    hAnpluCent = (TH2D*) file->Get("hAnpluCent");
    hCnpluCent = (TH2D*) file->Get("hCnpluCent");
    hAnminCent = (TH2D*) file->Get("hAnminCent");
    hCnminCent = (TH2D*) file->Get("hCnminCent");

    TCanvas *cAnpluCent = new TCanvas("cAnplu");
    // hAnpluCent->SetTitle("");
    hAnpluCent->ProfileX()->Draw();
    cAnpluCent->SaveAs("Plots/testPlots/AnpluCent.pdf");
 
    TCanvas *cCnpluCent = new TCanvas("cCnplu");
    // hAnpluCent->SetTitle("");
    hCnpluCent->ProfileX()->Draw();
    cCnpluCent->SaveAs("Plots/testPlots/CnpluCent.pdf");
 
    TCanvas *cAnminCent = new TCanvas("cAnmin");
    // hAnpluCent->SetTitle("");
    hAnminCent->ProfileX()->Draw();
    cAnminCent->SaveAs("Plots/testPlots/AnminCent.pdf");
 
    TCanvas *cCnminCent = new TCanvas("cCnmin");
    // hAnpluCent->SetTitle("");
    hCnminCent->ProfileX()->Draw();
    cCnminCent->SaveAs("Plots/testPlots/CnminCent.pdf");
